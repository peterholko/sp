...
use crate::constants::{
    DATABASE_MANAGER_ID, FOOD, FORAGE, GAME_ANIMAL, GAME_TICKS_PER_DAY, SPRING_WATER,
};
...
use crate::game::{
    ...
    VictoryState, BANDAGE_USE_TICKS, BURROW_SUPPLY_GOAL, STOCKADE_LOG_COST,
};
use crate::ids::{EntityObjMap, Ids};
use crate::item::{AttrKey, AttrVal, Inventory, Slot};
use crate::map::Map;
use crate::obj::{
    ActionProgress, ActiveTask, Assignment, Assignments, BuildUpgradeState, Class, ClassStructure,
    DroppedBag, HeroClass, Id, LastCombatTick, LastDamageTick, Misc, Name, Obj, Order, PlayerId,
    Portrait, Position, State, StateBuilding, StateDead, Stats, Subclass, SubclassHero,
    SubclassNPC, SubclassVillager, Template, TrueDeath, Viewshed, WorkEntry, WorkQueue, WorkStatus,
    WorkType, HERO_PORTRAITS, VILLAGER_PORTRAITS,
};
...
/// only the declared synthetic treatment and its derived Goblin-pressure change.
/// Assault composition and HP are matched, while actual per-leg spawn positions
/// are reported separately as geometry. This is neither a full-ECS nor an RNG
/// fingerprint.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize)]
...
fn expected_preparation_treatment_pressure(
    control: &PreparationPairLaunch,
    _treatment: &PreparationPairLaunch,
) -> i32 {
    // Every controlled treatment is readiness-only. Walls, equipment, and
    // healing must therefore leave threat pressure byte-for-byte unchanged.
    control.common_fingerprint.crisis_pressure
}
...
/// Declared Stockade, equipment, and healing treatments are readiness-only and
/// must not change Goblin threat pressure.
...
fn validate_preparation_pair_launches_with_wall_count(
    ...
    let expected_treatment_pressure = expected_preparation_treatment_pressure(control, treatment);
    if treatment.common_fingerprint.crisis_pressure != expected_treatment_pressure {
        return Err(format!(
            "unexpected treatment crisis pressure: expected={expected_treatment_pressure}; actual={}",
            treatment.common_fingerprint.crisis_pressure
        ));
    }
    let mut normalized_control_fingerprint = control.common_fingerprint.clone();
    let mut normalized_treatment_fingerprint = treatment.common_fingerprint.clone();
    normalized_control_fingerprint.crisis_pressure = 0;
    normalized_treatment_fingerprint.crisis_pressure = 0;
    if normalized_control_fingerprint != normalized_treatment_fingerprint {
        return Err(format!(
...
impl WorldView {
    ...
    pub fn has_built(&self, subclass: &str) -> bool {
        self.structures.iter().any(|s| {
            (s.subclass == subclass || (subclass == "campfire" && s.is_campfire_station()))
                && s.built
        })
    }
}
...
impl ItemView {
    ...
    // by an item whose name, class, or subclass equals it. The explicit
    // "Logs or Timber" requirement accepts either wood material.
    pub fn matches_req(&self, req_type: &str) -> bool {
        req_type == self.name
            || req_type == self.class
            || req_type == self.subclass
            || (req_type == "Logs or Timber" && (self.class == "Log" || self.class == "Timber"))
    }
}
...
impl StructureView {
    pub fn is_campfire_station(&self) -> bool {
        self.subclass == "campfire" || self.template == templates::SHELTER_TENT_TEMPLATE
    }
}
...
pub struct RunMetrics {
    ...
    pub num_deaths: u32,
    // Legacy PlayerObjectives score/report fields. Legendary victory remains
    // useful telemetry even though it is no longer a tutorial objective.
...
impl HeadlessGame {
    ...
    pub fn spawn_hero(&mut self, class: &str, name: &str) -> i32 {
        self.spawn_hero_with_portrait(class, name, crate::obj::default_hero_portrait())
    }

    fn spawn_hero_with_portrait(&mut self, class: &str, name: &str, portrait: &str) -> i32 {
        let pid = HEADLESS_PLAYER_ID;
        self.player_id = pid;

        // Deterministic client uuid (no RNG) — the game keys by player_id, the
        // uuid is only the client-map key.
        let client = Client::new(Uuid::from_u128(pid as u128), pid, self.packet_tx.clone());
    ...
    pub fn spawn_connected_scenario_helper(&mut self, name: &str) -> i32 {
        ...
        let helper_client = Client::new(
            Uuid::from_u128(helper_player_id as u128),
            helper_player_id,
    ...
    fn prepare_preparation_launch_internal(
        ...
        use crate::game::{
            CrisisKind, InitialEncounterState, ASSAULT_READY_GRACE_TICKS, GOBLIN_PREPARING_PRESSURE,
        };
        ...
        // Keep the shared baseline structure identical. A completed Stockade
        // is readiness-only, so the fixture may differ without advancing the
        // crisis clock or changing threat pressure.
    ...
    fn install_preparation_inventory(&mut self, add_bandage: bool) -> Result<i32, String> {
        ...
        {
            inventory.new(potion_id, "Health Potion".to_string(), 1, &templates);
        }
    ...
    pub fn protected_intro_snapshot(&self) -> ProtectedIntroSnapshot {
        ...
        ProtectedIntroSnapshot {
            ...
            opening_rat_ambush_armed: initial.opening_rat_ambush_armed,
    ...
    pub fn observe_for_player(&mut self, player_id: i32) -> WorldView {
        ...
        let structures = {
            ...
            q.iter(world)
                .filter(|(_, p, _, _, _, _, _)| p.0 == pid)
                .map(
                    |(id, _p, pos, template, subclass, state, inv)| StructureView {
                        id: id.0,
                        pos: *pos,
                        template: template.0.clone(),
                        subclass: subclass.to_string(),
...
mod tests {
    ...
    fn assert_equipped_tattered_clothing(inventory: &Inventory) {
        for (name, expected_slot) in [
            ("Tattered Shirt", Slot::Chest),
            ("Tattered Pants", Slot::Pants),
        ] {
            let item = inventory
                .items
                .iter()
                .find(|item| item.name == name)
                .unwrap_or_else(|| panic!("villager should start with {name}"));
            assert_eq!(item.quantity, 1);
            assert_eq!(item.slot, Some(expected_slot));
            assert!(item.equipped, "{name} should start equipped");
            assert_eq!(item.owner, inventory.owner);
        }
    }

    fn hero_item_quantity(game: &HeadlessGame, player_id: i32, item_name: &str) -> i32 {
    ...
    fn prepare_full_crisis_progression_facts(game: &mut HeadlessGame) {
        ...
        // Villagers and sanctuary strength are readiness evidence only; they
        // deliberately remain in this fixture without contributing threat.
    ...
    fn investigate_shipwreck_for_smoke(game: &mut HeadlessGame, player_id: i32) -> i32 {
        ...
        // Most callers exercise post-search systems rather than the ambush.
        // Preserve the original distant deadline so the ordinary investigation
        // can complete; the dedicated opening-ambush regression below exercises
        // the production first-attempt interruption without this fixture hold.
        game.app
            .world_mut()
            .resource_mut::<crate::game::InitialEncounterState>()
            .get_mut(&player_id)
            .expect("headless initial encounter")
            .opening_rat_ambush_armed = true;
    ...
    #[test]
    fn fresh_shipwreck_contains_one_optional_foraging_kit() {
        let mut game = HeadlessGame::new(10_000);
        let player_id = game.spawn_hero("Warrior", "ForagingKitBot");
        let shipwreck_id = run_shipwreck_id(&mut game, player_id);
        let shipwreck_entity = game
            .world()
            .resource::<EntityObjMap>()
            .get_entity(shipwreck_id)
            .expect("run-owned Shipwreck entity");
        let inventory = game
            .world()
        ...
        let kit = inventory
            .items
            .iter()
            .find(|item| item.name == "Foraging Kit")
            .expect("starter Foraging Kit");

        assert_eq!(kit.quantity, 1);
        assert_eq!(kit.attr_num(&AttrKey::Foraging), 2.0);
        assert!(kit.durability.is_some());
    }

    #[test]
    fn equipped_foraging_kit_starts_an_authoritative_eight_second_gather() {
        let mut game = HeadlessGame::new(10_000);
        let player_id = game.spawn_hero("Warrior", "ForagingTimerBot");

        let (hero_id, hero_entity, hero_pos, item_id, item_templates) = {
            let world = game.app.world_mut();
            ...
            let hero_entity = world
                ...
                .get_entity(hero_id)
                .unwrap();
            let hero_pos = *world.get::<Position>(hero_entity).unwrap();
            let item_id = world.resource_mut::<Ids>().new_item_id();
            let item_templates = world.resource::<Templates>().item_templates.clone();
            (hero_id, hero_entity, hero_pos, item_id, item_templates)
        };

        {
            let world = game.app.world_mut();
            let mut inventory = world.get_mut::<Inventory>(hero_entity).unwrap();
            for item in &mut inventory.items {
                if item.slot == Some(Slot::MainHand) {
                    item.equipped = false;
                }
            }
            inventory.new(item_id, "Foraging Kit".to_string(), 1, &item_templates);
            inventory
                .items
                .iter_mut()
                .find(|item| item.id == item_id)
            ...
            world
                ...
                .or_default()
                .insert(
                    "Useful Underbrush".to_string(),
                    Resource {
                        name: "Useful Underbrush".to_string(),
                        image: "plantfibers".to_string(),
                        ...
                        quantity: 10,
                        properties: Vec::new(),
                        produces: Some(vec!["Plant Fibers".to_string()]),
                        reveal: true,
                    },
                );
        }
        ...
        game.start_packet_capture();
        game.inject(PlayerEvent::Gather {
            player_id,
            res_type: FORAGE.to_string(),
        });
        game.tick(3);
        ...
        assert!(packets
            .iter()
            .any(|packet| matches!(packet, ResponsePacket::Gather { gather_time: 8 })));
        let progress = game
            .world()
            .get::<ActionProgress>(hero_entity)
            .expect("authoritative hero gather progress");
        assert_eq!(
            progress.end_tick - progress.start_tick,
            8 * crate::constants::TICKS_PER_SEC
        );
        ...
        let event = game
            ...
            .find(|event| {
                matches!(
                    &event.event_type,
                    GameEventType::GatherEvent { gatherer_id, res_type }
                        if *gatherer_id == hero_id && res_type == FORAGE
                )
            })
            .expect("scheduled Forage event");
        assert_eq!(event.event_id, progress.action_id);
        assert_eq!(
            event.run_tick - event.start_tick,
            8 * crate::constants::TICKS_PER_SEC
        );
    }

    #[test]
    fn hero_gathering_repeats_with_new_progress_until_the_hero_moves() {
        let mut game = HeadlessGame::new(10_000);
        let player_id = game.spawn_hero("Warrior", "ContinuousGatherBot");

        let (hero_id, hero_entity, hero_pos) = {
            let world = game.app.world_mut();
            let hero_id = world.resource::<Ids>().get_hero(player_id).unwrap();
            let hero_entity = world
                .resource::<EntityObjMap>()
        ...
        assert_eq!(
            game.world()
                ...
                .count(),
            1,
            "same-tick duplicate input must not create parallel gather loops"
        );

        let first_progress = *game
            .world()
            .get::<ActionProgress>(hero_entity)
            .expect("first authoritative gather cycle");
        ...
        game.tick(
            first_progress
                ...
                .saturating_add(3) as u32,
        );

        let repeated_progress = *game
            .world()
            .get::<ActionProgress>(hero_entity)
            .expect("repeated authoritative gather cycle");
        let repeated_packets = game.finish_packet_capture();
        let repeated_duration = (repeated_progress.end_tick - repeated_progress.start_tick)
            / crate::constants::TICKS_PER_SEC;
        assert_ne!(repeated_progress.action_id, first_progress.action_id);
        assert!(repeated_packets.iter().any(|packet| matches!(
            packet,
            ResponsePacket::Gather { gather_time } if *gather_time == repeated_duration
        )), "each automatically repeated hero gather cycle must restart the gather-button countdown");
        assert_eq!(
            game.world().get::<State>(hero_entity),
            Some(&State::Gathering)
        );
        assert!(game.world().resource::<GameEvents>().values().any(|event| {
            matches!(
                &event.event_type,
                GameEventType::GatherEvent { gatherer_id, res_type }
                    if *gatherer_id == hero_id && res_type == FORAGE
        ...
        let destination = passable_unoccupied_adjacent_position(&mut game, hero_pos);
        ...
        assert!(game.world().get::<ActionProgress>(hero_entity).is_none());
        assert!(game.world().resource::<GameEvents>().values().all(|event| {
            !matches!(
                &event.event_type,
                GameEventType::GatherEvent { gatherer_id, .. } if *gatherer_id == hero_id
            )
        }));
    }
    ...
    fn queue_shipwreck_investigation(game: &mut HeadlessGame, player_id: i32) -> (i32, i32) {
        ...
        // These focused interruption fixtures provide their own combat/damage
        // source. Hold the opening ambush at its original distant deadline so
        // it cannot race the condition under test.
        game.app
            .world_mut()
            .resource_mut::<crate::game::InitialEncounterState>()
            .get_mut(&player_id)
    ...
    fn spawn_explicit_completed_structure(
        ...
        let origin = game
            .observe_for_player(player_id)
            .hero
            .unwrap_or_else(|| panic!("hero for explicit {template_name} fixture"))
            .pos;
        let occupied = {
            let world = game.app.world_mut();
            let mut positioned = world.query::<&Position>();
        ...
        let anchor = Map::range((origin.x, origin.y), 3)
            ...
            .unwrap_or_else(|| panic!("nearby unoccupied {template_name} fixture tile"));
        game.spawn_completed_preparation_structure(template_name, anchor)
            .unwrap_or_else(|error| panic!("explicit completed {template_name} fixture: {error}"));
        anchor
    }

    fn spawn_explicit_completed_burrow(game: &mut HeadlessGame, player_id: i32) -> Position {
        spawn_explicit_completed_structure(game, player_id, "Burrow")
    }

    fn living_pre_crisis_units(
        game: &mut HeadlessGame,
        player_id: i32,
        slot: crate::game::PreCrisisSkirmishSlot,
    ) -> Vec<(i32, Position)> {
        let world = game.app.world_mut();
        let mut query = world.query::<(
            &Id,
            &Position,
            &crate::game::PreCrisisSkirmishUnit,
            Option<&StateDead>,
        )>();
        query
            .iter(world)
            .filter(|(_, _, skirmish, dead)| {
                skirmish.owner_player_id == player_id && skirmish.slot == slot && dead.is_none()
            })
            .map(|(id, pos, _, _)| (id.0, *pos))
            .collect()
    }
    ...
    fn resolve_goblin_normally_for_undead_smoke(game: &mut HeadlessGame) -> u64 {
        ...
        use crate::game::{
            ...
            GOBLIN_ONLINE_PRESSURE_TIER_THREE_TICKS, GOBLIN_PREPARATION_WINDOW_TICKS,
            GOBLIN_PRESSURE_MIN_ONLINE_TICKS,
        };
        ...
        for (minimum_ticks, expected_phase) in [
            ...
            (GOBLIN_PREPARATION_WINDOW_TICKS, CrisisPhase::AssaultReady),
        ] {
    ...
    #[test]
    fn ready_combo_from_killing_blow_can_hit_the_next_target() {
        use crate::combat::{AttackType, ComboTracker, TRANSFERABLE_COMBO_TARGET_ID};
        use crate::ids::EntityObjMap;

        let mut game = HeadlessGame::new(2_000);
        let player_id = game.spawn_hero("Warrior", "ComboCarryBot");
        let (hero_entity, hero_id, hero_position) = {
            let world = game.app.world_mut();
            let mut heroes =
                world.query_filtered::<(Entity, &Id, &PlayerId, &Position), With<SubclassHero>>();
            heroes
                .iter(world)
                .find(|(_, _, owner, _)| owner.0 == player_id)
                .map(|(entity, id, _, position)| (entity, id.0, *position))
                .expect("combo carry hero")
        };
        let defeated_target_id = game.spawn_safe_logout_test_hostile(hero_position);
        let next_target_id = game.spawn_safe_logout_test_hostile(hero_position);
        let (defeated_target_entity, next_target_entity) = {
            let entity_map = game.world().resource::<EntityObjMap>();
            (
                entity_map
                    .get_entity(defeated_target_id)
                    .expect("first combo target"),
                entity_map
                    .get_entity(next_target_id)
                    .expect("second combo target"),
            )
        };
        let combo_tick = game.game_tick();
        {
            let world = game.app.world_mut();
            world
                .get_mut::<Stats>(hero_entity)
                .expect("combo carry hero stats")
                .base_damage = Some(20);
            world
                .get_mut::<Stats>(defeated_target_entity)
                .expect("first combo target stats")
                .hp = 1;
            world.entity_mut(hero_entity).insert(ComboTracker {
                target_id: defeated_target_id,
        ...
        game.tick(1);
        let killing_blow_packets = game.finish_packet_capture();

        assert_eq!(
            game.world().get::<State>(defeated_target_entity).copied(),
            Some(State::Dead)
        );
        let tracker = game
            .world()
            .get::<ComboTracker>(hero_entity)
            .expect("ready transferable combo");
        assert_eq!(tracker.target_id, TRANSFERABLE_COMBO_TARGET_ID);
        assert_eq!(tracker.attacks, vec![AttackType::Quick, AttackType::Quick]);
        assert!(killing_blow_packets.iter().any(|packet| matches!(
            packet,
            ResponsePacket::CombatState {
                version: 3,
                target_id,
                available_finisher: Some(finisher),
                finisher_transferable: true,
                ..
            } if *target_id == defeated_target_id && finisher == "Hamstring"
        )));

        let next_hp_before = game
            .world()
            .get::<Stats>(next_target_entity)
            .expect("second combo target stats")
            .hp;
        ...
        assert!(
            game.world()
                .get::<Stats>(next_target_entity)
                .expect("second target after carried combo")
                .hp
                < next_hp_before,
            "the carried finisher must damage the newly selected target"
        );
        assert!(game
            .world()
            .get::<ComboTracker>(hero_entity)
            .expect("combo tracker after finisher")
            .attacks
            .is_empty());
    }
    ...
    #[test]
    fn character_portraits_persist_on_spawned_heroes_and_villagers() {
        let selected_portrait = HERO_PORTRAITS[3];
        let mut game = HeadlessGame::new(1_000);
        let player_id =
            game.spawn_hero_with_portrait("Ranger", "PortraitIdentityBot", selected_portrait);

        let hero_portrait = {
            let world = game.app.world_mut();
            let mut query = world.query_filtered::<(&PlayerId, &Portrait), With<SubclassHero>>();
            query
                .iter(world)
                .find(|(owner, _)| owner.0 == player_id)
                .map(|(_, portrait)| portrait.0.clone())
                .expect("spawned hero portrait")
        };
        assert_eq!(hero_portrait, selected_portrait);

        let villager_id = game
            .spawn_connected_scenario_villager(player_id)
            .expect("spawn ordinary owner villager");
        let villager_portrait = {
            let world = game.world();
            let entity = world
                .resource::<EntityObjMap>()
                .get_entity(villager_id)
                .expect("spawned villager entity");
            world
                .get::<Portrait>(entity)
                .expect("spawned villager portrait")
                .0
                .clone()
        };
        assert!(VILLAGER_PORTRAITS.contains(&villager_portrait.as_str()));
    }

    #[test]
    fn ordinary_villager_starts_with_equipped_tattered_clothing() {
        let mut game = HeadlessGame::new(1_000);
        let player_id = game.spawn_hero("Warrior", "ClothedVillagerBot");
        let villager_id = game
            .spawn_connected_scenario_villager(player_id)
            .expect("spawn ordinary owner villager");
        ...
        let inventory = world
            .get::<Inventory>(entity)
            .expect("spawned villager inventory");

        assert_equipped_tattered_clothing(inventory);
    }
    ...
    fn revised_opening_starter_setup_is_exact_for_every_class() {
        ...
        for (class, class_salvage) in [
            ...
            {
                let world = game.app.world_mut();
                let mut map_corpse_query = world.query::<&crate::obj::Template>();
                let map_corpse_count = map_corpse_query
                    .iter(world)
                    .filter(|template| template.0 == "Human Corpse")
                    .count();
                assert_eq!(
                    map_corpse_count, 0,
                    "Human Corpses should be Shipwreck items, not map objects"
                );
            }

            {
                ...
                assert_eq!(
                    hero_items,
                    ["Sharpened Stick", "Tattered Pants", "Tattered Shirt"]
                );
                assert_eq!(
                    hero_inventory
                        .items
                ...
                let corpse_items = shipwreck_inventory
                    .items
                    .iter()
                    .filter(|item| item.name == "Human Corpse")
                    .collect::<Vec<_>>();
                assert_eq!(corpse_items.len(), 2);
                assert_ne!(corpse_items[0].id, corpse_items[1].id);
                assert!(corpse_items.iter().all(|item| {
                    item.quantity == 1
                        && item.class == crate::item::CORPSE_ITEM
                        && item.image == "humancorpse"
                        && item.weight == 75.0
                }));
                assert!(matches!(
                    shipwreck_inventory
                        ...
                        .and_then(|item| item.attrs.get(&AttrKey::Healing)),
                    Some(AttrVal::Num(value)) if *value == 50.0
                ));
                let mut actual_manifest = BTreeMap::new();
                for item in &shipwreck_inventory.items {
                    if let Some(quantity) = actual_manifest.get_mut(&item.name) {
                        assert!(
                            matches!(item.name.as_str(), "Human Corpse" | "Crude Hatchet"),
                            "starter manifest unexpectedly duplicated {}",
                            item.name
                        );
                        *quantity += item.quantity;
                    } else {
                        actual_manifest.insert(item.name.clone(), item.quantity);
                    }
                ...
                let recipes = world.resource::<crate::recipe::Recipes>();
                assert_eq!(
                    recipes
                        .get_basic_recipes_packet(player_id)
                        .into_iter()
                ...
                for recipe_name in [
                    ...
                    assert!(
                        recipes
                            .get_for_owner_by_name(player_id, recipe_name)
                            .is_some(),
                        "{class} is missing foundational recipe {recipe_name}"
                    );
                }
                for recipe_name in [
                    "Fishing Rod",
                    ...
                    assert!(
                        recipes
                            .get_for_owner_by_name(player_id, recipe_name)
                            .unwrap_or_else(|| panic!("missing Crafting Tent recipe {recipe_name}"))
                            .supports_structure("Crafting Tent"),
                        "{recipe_name} should be made at the Crafting Tent"
                    );
                }
                assert!(recipes
                    .get_for_owner_by_name(player_id, "Hide Cap")
                    .expect("primitive armor recipe")
                    .supports_structure("Crafting Tent"));
                assert!(recipes
                    .get_for_owner_by_name(player_id, "Copper Spear")
                    .expect("foundational copper recipe")
                    .supports_structure("Blacksmith"));
                assert!(recipes
                    .get_for_owner_by_name(player_id, "Hunting Bow")
                    .expect("foundational bow recipe")
                    .supports_structure("Workshop"));

                let starter_stick = hero_inventory
                    .items
                    .iter()
                    .find(|item| item.name == "Sharpened Stick")
                    .expect("starter stick");
                assert!(!starter_stick.equipped);
                let starter_hatchets = shipwreck_inventory
                    .items
                    .iter()
                    .filter(|item| item.name == "Crude Hatchet")
                    .collect::<Vec<_>>();
                assert_eq!(starter_hatchets.len(), 2);
                assert!(starter_hatchets.iter().all(|item| item.quantity == 1));
                for starter_hatchet in starter_hatchets {
                    assert!(matches!(
                ...
                assert_eq!(inventory.items[0].quantity, 30);
            ...
            assert_eq!(
                plan_names,
                ["Campfire", "Crafting Tent", "Well"],
                "the rescued-villager deed chain must not remain pre-unlocked"
            );
        }
    }

    #[test]
    fn successful_starter_campfire_meat_craft_completes_tutorial_objective() {
        let mut game = HeadlessGame::new(1_000);
        let player_id = game.spawn_hero("Warrior", "CampfireCookingTutorialBot");
        let hero_id = game
            .world()
            .resource::<Ids>()
            .get_hero(player_id)
        ...
        let (hero_entity, campfire_entity, campfire_id, campfire_pos) = {
            let world = game.app.world_mut();
            let hero_entity = world
                .resource::<EntityObjMap>()
                .get_entity(hero_id)
                .expect("headless hero entity");
            let mut campfires = world.query_filtered::<
                (Entity, &Id, &PlayerId, &Template, &Position),
                With<ClassStructure>,
            >();
            let (campfire_entity, campfire_id, _, _, campfire_pos) = campfires
                .iter(world)
                .find(|(_, _, owner, template, _)| owner.0 == player_id && template.0 == "Campfire")
                .expect("starter Campfire");
            (hero_entity, campfire_entity, campfire_id.0, *campfire_pos)
        };

        {
            ...
            *world
                .get_mut::<Position>(hero_entity)
                .expect("headless hero position") = campfire_pos;
            let item_templates = world.resource::<Templates>().item_templates.clone();
            let raw_meat_id = world.resource_mut::<Ids>().new_item_id();
            world
                .get_mut::<Inventory>(campfire_entity)
                .expect("starter Campfire inventory")
                .new(
                    raw_meat_id,
                    "Bristleback Raw Meat".to_string(),
                    1,
                    &item_templates,
                );
            assert_eq!(
                Obj::get_capacity(
                    &"Campfire".to_string(),
                    &world.resource::<Templates>().obj_templates,
                ),
                40
            );
            assert_eq!(
                world
                    ...
                    .expect("fueled Campfire inventory")
                    .get_total_weight(),
                31
            );
        }
        ...
        assert!(game
            ...
            .resource::<Objectives>()
            .get(&player_id)
            .is_none_or(|objectives| !objectives.cook_animal_meat));

        game.start_packet_capture();
        game.inject(PlayerEvent::StructureCraft {
            player_id,
        ...
        assert!(
            !game
                ...
                .resource::<Objectives>()
                .get(&player_id)
                .is_some_and(|objectives| objectives.cook_animal_meat),
            "starting the craft must not complete the tutorial objective"
        );

        game.tick(25);
        let cooking_packets = game.finish_packet_capture();
        assert!(
            game.world()
                .get::<Inventory>(campfire_entity)
                .expect("Campfire inventory after cooking")
                .items
                .iter()
                .any(|item| item.subclass == "Cooked Meat"),
            "the real Campfire craft did not finish; packets: {cooking_packets:?}"
        );
        assert!(
            game.world()
                .resource::<Objectives>()
                .get(&player_id)
                .expect("objectives after cooking")
                .cook_animal_meat
        );
    }

    #[test]
    fn scripted_necromancer_consumes_shipwreck_corpse_items_and_raises_sailors() {
        use crate::game::InitialEncounterState;

        let mut game = HeadlessGame::new(2_000);
        let player_id = game.spawn_hero("Warrior", "ShipwreckCorpseNecromancerBot");
        let shipwreck_id = run_shipwreck_id(&mut game, player_id);

        let (shipwreck_entity, shipwreck_pos, necromancer_entity, necro_home, corpse_item_ids) = {
            let world = game.app.world();
            let entity_map = world.resource::<EntityObjMap>();
            let shipwreck_entity = entity_map
                .get_entity(shipwreck_id)
                .expect("run Shipwreck entity");
            let shipwreck_pos = *world
                .get::<Position>(shipwreck_entity)
                .expect("run Shipwreck position");
            let encounter = world
                .resource::<InitialEncounterState>()
                .get(&player_id)
                .expect("run encounter state");
            let necromancer_entity = entity_map
                .get_entity(encounter.necromancer_id)
                .expect("dormant run Necromancer");
            let corpse_item_ids = world
                .get::<Inventory>(shipwreck_entity)
                .expect("run Shipwreck inventory")
                .items
                .iter()
                .filter(|item| item.name == "Human Corpse")
                .map(|item| item.id)
            ...
            (
                ...
                shipwreck_pos,
                necromancer_entity,
                encounter.necro_home,
                corpse_item_ids,
            )
        };
        assert_eq!(corpse_item_ids.len(), 2);

        {
            let world = game.app.world_mut();
            *world
                .get_mut::<Position>(necromancer_entity)
                .expect("Necromancer position") = shipwreck_pos;
            *world
                .get_mut::<State>(necromancer_entity)
                .expect("Necromancer state") = State::None;
        }
        {
            let mut commands = game.app.world_mut().commands();
        ...
        game.app.world_mut().flush();

        let first_raise = (0..100)
            .find_map(|_| {
                game.tick(1);
                game.world()
                    .resource::<MapEvents>()
                    ...
                    .find(|event| {
                        matches!(
                            ...
                            VisibleEvent::SpellRaiseDeadEvent {
                                corpse_id,
                                corpse_item_id: Some(_),
                            } if corpse_id == shipwreck_id
                        )
                    })
                    .cloned()
            })
            .expect("Necromancer schedules Shipwreck Raise Dead");
        let first_corpse_item_id = match first_raise.event_type {
            VisibleEvent::SpellRaiseDeadEvent {
                corpse_item_id: Some(item_id),
                ..
            } => item_id,
            _ => unreachable!(),
        };
        assert!(corpse_item_ids.contains(&first_corpse_item_id));

        // A stale duplicate for the same exact item must fail after the first
        // event consumes it, rather than silently consuming the second body.
        let duplicate_event_id = Uuid::from_u128(9_009_009_009);
        let mut duplicate_raise = first_raise;
        duplicate_raise.event_id = duplicate_event_id;
        game.app
            .world_mut()
            .resource_mut::<MapEvents>()
        ...
        let first_raised = (0..100).any(|_| {
            game.tick(1);
            game.world()
                .get::<Inventory>(shipwreck_entity)
                .expect("Shipwreck remains after first Raise Dead")
                .items
                .iter()
                .filter(|item| item.name == "Human Corpse")
                .count()
        ...
        assert!(first_raised, "Necromancer did not consume the first corpse");
        let remaining_after_duplicate = game
            .world()
            .get::<Inventory>(shipwreck_entity)
            .expect("Shipwreck inventory after duplicate Raise Dead")
            .items
            .iter()
            .filter(|item| item.name == "Human Corpse")
            .map(|item| item.id)
            .collect::<HashSet<_>>();
        assert_eq!(remaining_after_duplicate.len(), 1);
        assert!(!remaining_after_duplicate.contains(&first_corpse_item_id));

        let raised_both = (0..400).any(|_| {
            game.tick(1);
            let world = game.app.world_mut();
            let remaining_corpse_ids = world
                .get::<Inventory>(shipwreck_entity)
                .expect("Shipwreck remains after Raise Dead")
                .items
                .iter()
                .filter(|item| item.name == "Human Corpse")
                .map(|item| item.id)
                .collect::<HashSet<_>>();
            let mut raised_sailor_query = world.query::<&Template>();
            let raised_sailors = raised_sailor_query
                .iter(world)
                .filter(|template| template.0 == "Shipwreck Zombie")
                .count();
            remaining_corpse_ids.is_empty() && raised_sailors >= 2
        });

        assert!(
            raised_both,
            "Necromancer did not raise both Shipwreck sailors"
        );
        assert!(game
            .world()
            .resource::<EntityObjMap>()
            .get_obj_by_entity(shipwreck_entity)
            .is_some());
        assert!(corpse_item_ids.iter().all(|corpse_item_id| {
            game.world()
                .get::<Inventory>(shipwreck_entity)
                .expect("Shipwreck inventory after raising")
                .items
                .iter()
                .all(|item| item.id != *corpse_item_id)
        }));
    }

    #[test]
    fn revised_opening_first_objective_completes_once_when_stick_is_equipped() {
        let mut game = HeadlessGame::new(1_000);
        let player_id = game.spawn_hero("Warrior", "StarterWeaponTutorialBot");
        ...
        assert!(
            game.world()
                .resource::<Objectives>()
                .get(&player_id)
                .expect("starter objective history")
                .equip_sharpened_stick
        );
        ...
        assert_eq!(
            capture_current_objective_id(&mut game),
            "scavenge_shipwreck",
            "switching weapons later must not reopen the completed tutorial step"
        );
    }

    #[test]
    fn revised_post_rescue_forest_prospect_requires_survivor_history() {
        let mut game = HeadlessGame::new(1_000);
        let player_id = game.spawn_hero("Warrior", "ForestProspectTutorialBot");
        let forest_pos = {
            let world = game.app.world_mut();
            let resources = world.resource::<Resources>();
            resources
                .iter()
                .find_map(|(pos, resources_on_tile)| {
                    resources_on_tile
                        .values()
                        .any(|resource| resource.res_type == crate::constants::LOG)
                        .then_some(*pos)
                })
                .expect("generated forest resource tile")
        };
        let hero_entity = {
            let world = game.app.world_mut();
            let hero_id = world.resource::<Ids>().get_hero(player_id).unwrap();
        ...
        *game
        ...
        game.inject(PlayerEvent::Prospect { player_id });
        game.tick(55);
        assert!(
            !game
                .world()
        ...
        assert!(
            game.world()
                .resource::<Objectives>()
                .get(&player_id)
                .expect("post-rescue objectives")
                .prospect_forest
        );
    }

    #[test]
    fn prospecting_progress_is_authoritative_across_reconnect_and_completion() {
        let mut game = HeadlessGame::new(1_000);
        let player_id = game.spawn_hero("Warrior", "ProspectingProgressBot");
        let (hero_id, hero_entity) = {
            let world = game.app.world_mut();
            let hero_id = world.resource::<Ids>().get_hero(player_id).unwrap();
            let hero_entity = world
                .resource::<EntityObjMap>()
        ...
        let initial_progress = *game
            .world()
            .get::<ActionProgress>(hero_entity)
            .expect("prospecting action progress");
        let start_packets = game.finish_packet_capture();
        assert!(
            start_packets.iter().any(|packet| match packet {
                ResponsePacket::PerceptionChanges { events } => events.iter().any(|event| {
                    let crate::network::ChangeEvents::ObjUpdate { obj_id, attrs, .. } = event
                    else {
                        return false;
                    };
                    *obj_id == hero_id
                        && attrs.iter().any(|attr| {
                            attr.attr == "action_id"
                                && attr.value == initial_progress.action_id.to_string()
                        })
                        && attrs
                            .iter()
                            .any(|attr| attr.attr == "action_duration_ms" && attr.value == "5000")
                        && attrs
                            .iter()
                            .any(|attr| attr.attr == "action_elapsed_ms" && attr.value == "0")
                }),
                _ => false,
            }),
            "prospecting must immediately publish its authoritative timeline"
        );
        assert_eq!(
            initial_progress.end_tick - initial_progress.start_tick,
            5 * crate::constants::TICKS_PER_SEC
        );
        assert_eq!(
            game.world().get::<State>(hero_entity),
            Some(&State::Prospecting)
        );
        ...
        let reconnect_hero = reconnect_packets
            .iter()
            .find_map(|packet| match packet {
                ResponsePacket::InitPerception { data }
                | ResponsePacket::NewPerception { data } => data
                    .observers
                    .iter()
                    .find(|observer| observer.id == hero_id)
                    .cloned(),
                _ => None,
            })
            .expect("reconnect perception containing the prospecting hero");

        assert_eq!(reconnect_hero.action_id, Some(initial_progress.action_id));
        assert_eq!(reconnect_hero.action_duration_ms, Some(5_000));
        assert!(
            reconnect_hero
                .action_elapsed_ms
                .is_some_and(|elapsed| elapsed > 0 && elapsed < 5_000),
            "reconnect must resume the existing prospecting action instead of restarting it"
        );

        let current_tick = game.world().resource::<GameTick>().0;
        let ticks_to_completion = initial_progress
            .end_tick
            .saturating_sub(current_tick)
            .saturating_add(2) as u32;
        game.tick(ticks_to_completion);

        assert!(game.world().get::<ActionProgress>(hero_entity).is_none());
        assert_eq!(game.world().get::<State>(hero_entity), Some(&State::None));
    }

    #[test]
    fn moving_interrupts_prospecting_and_clears_authoritative_progress() {
        let mut game = HeadlessGame::new(1_001);
        let player_id = game.spawn_hero("Warrior", "ProspectingInterruptBot");
        let (hero_id, hero_entity, hero_pos) = {
            let world = game.app.world_mut();
            let hero_id = world.resource::<Ids>().get_hero(player_id).unwrap();
            let hero_entity = world
        ...
        let occupied = game.observe().occupied;
        let destination = Map::range((hero_pos.x, hero_pos.y), 1)
            .into_iter()
            .map(|(x, y)| Position { x, y })
            .find(|position| {
                Map::is_adjacent_excluding_source(*position, hero_pos)
                    && Map::is_passable(position.x, position.y, game.map())
                    && !occupied.contains(&(position.x, position.y))
            })
            .expect("open adjacent tile for prospecting interruption");

        game.inject(PlayerEvent::Prospect { player_id });
        game.tick(2);
        assert!(game.world().get::<ActionProgress>(hero_entity).is_some());
        ...
        assert!(game
            ...
            .resource::<MapEvents>()
            .values()
            .all(|event| event.obj_id != hero_id
                || !matches!(event.event_type, VisibleEvent::ProspectEvent)));
    }

    #[test]
    fn revised_hero_rabbit_skinning_reports_activity_and_records_hide_tutorial_completion() {
        let mut game = HeadlessGame::new(1_000);
        let player_id = game.spawn_hero("Warrior", "CarcassRefineTutorialBot");
        let (hero_id, hero_entity, carcass_id) = {
            let world = game.app.world_mut();
            let hero_id = world.resource::<Ids>().get_hero(player_id).unwrap();
            let hero_entity = world
                .resource::<EntityObjMap>()
                ...
                .unwrap();
            let item_templates = world.resource::<Templates>().item_templates.clone();
            let carcass_id = world.resource_mut::<Ids>().new_item_id();
            world.get_mut::<Inventory>(hero_entity).unwrap().new(
                carcass_id,
                "Felled Swiftstep Hare".to_string(),
                1,
                &item_templates,
            ...
            (hero_id, hero_entity, carcass_id)
        };

        game.start_packet_capture();
        game.inject(PlayerEvent::Refine {
            player_id,
            item_id: carcass_id,
        });
        game.tick(1);
        ...
        let progress = game
            .world()
            .get::<ActionProgress>(hero_entity)
            .expect("carcass refining action progress");
        assert!(progress.end_tick > progress.start_tick);
        assert_eq!(
            game.world().get::<State>(hero_entity),
            Some(&State::Refining)
        );
        assert_eq!(
            game.world().get::<ActiveTask>(hero_entity),
            Some(&ActiveTask::Skinning)
        );
        assert!(start_packets.iter().any(|packet| matches!(
            packet,
            ResponsePacket::InfoActivityUpdate { id, activity }
                if *id == hero_id && activity == "Skinning"
        )));
        ...
        assert_eq!(
            game.world().get::<ActiveTask>(hero_entity),
            Some(&ActiveTask::None)
        );
        let inventory = game.world().get::<Inventory>(hero_entity).unwrap();
        assert!(inventory
            .items
            .iter()
            .any(|item| item.name == "Swiftstep Raw Hide" && item.quantity > 0));
        assert!(
            game.world()
                .resource::<Objectives>()
                .get(&player_id)
                .expect("post-refine objectives")
                .refine_animal_carcass
        );
    }

    #[test]
    fn cancelling_hero_carcass_refine_removes_action_progress() {
        let mut game = HeadlessGame::new(1_001);
        let player_id = game.spawn_hero("Warrior", "CarcassRefineCancelBot");
        let (hero_entity, carcass_id) = {
            let world = game.app.world_mut();
            let hero_id = world.resource::<Ids>().get_hero(player_id).unwrap();
            let hero_entity = world
                .resource::<EntityObjMap>()
        ...
        game.inject(PlayerEvent::CancelAction { player_id });
        game.tick(1);
    ...
    fn campfire_burnout_clears_stationary_fire_centered_visibility() {
        ...
        let fuel_cycle = crate::constants::FIREWOOD_BURN_TICKS;
    ...
    #[test]
    fn shelter_tent_campfire_light_has_owner_radius_one_but_is_not_public() {
        use crate::obj::Campfire;
        ...
        assert!(active_campfire_light_ids(&game, player_id).contains(&structure_id));
        let (_, _, owner_tent_vision, owner_tent_light) = primary_hero_light_snapshot(&mut game);
        assert!(
            owner_tent_light.is_none(),
            "Shelter Tent light is a fire-centered observer rather than personal hero vision"
        );
        assert_eq!(owner_tent_vision, base_owner_vision);

        let owner_adjacent_pos = passable_unoccupied_adjacent_position(&mut game, structure_pos);
        *game
            .app
            .world_mut()
            .get_mut::<Position>(owner_hero_entity)
            .expect("owner hero position") = owner_adjacent_pos;
        game.tick(1);
        assert!(
            active_campfire_light_ids(&game, player_id).contains(&structure_id),
            "the owner adjacent to the Shelter Tent retains its radius-one light bubble"
        );
        ...
        assert!(
            ...
            "moving away from the Shelter Tent must not leave personal CampfireLight behind"
        );
        assert_eq!(owner_away_vision, base_owner_vision);
        assert!(
            !active_campfire_light_ids(&game, player_id).contains(&structure_id),
            "the owner away from the Shelter Tent has no retained light bubble"
        );
        assert!(
    ...
    fn foreign_adjacent_hero_can_light_standalone_campfire_only_once() {
        ...
        game.tick(4);
        let ignition_packets = game.finish_packet_capture();
        ...
        assert_eq!(
            durability_after,
            durability_before - 1,
            "the accepted activation consumes exactly one ignition durability"
        );
        assert!(durability_before > durability_after);
        assert!(
            ignition_packets.iter().any(|packet| matches!(
                packet,
                ResponsePacket::InfoItemsUpdate {
                    id,
                    items_updated,
                    ..
                } if *id == helper_hero_id
                    && items_updated.iter().any(|item| {
                        item.class == crate::constants::IGNITION_TOOL
                            && item.durability == Some(durability_after)
                    })
            )),
            "Campfire ignition must immediately synchronize Flint durability"
        );
    ...
    #[test]
    fn handheld_torch_ignition_is_carrier_scoped_durable_and_idempotent() {
        let mut game = HeadlessGame::new(10_000);
        let player_id = game.spawn_hero("Warrior", "TorchIgnitionBot");
        let hero_id = game
            .world()
            .resource::<Ids>()
            .get_hero(player_id)
        ...
        let hero_entity = game
            ...
            .expect("headless hero entity");
        let shipwreck_id = run_shipwreck_id(&mut game, player_id);
        let (torch_stack_id, flint_id) = {
            let world = game.app.world_mut();
            let shipwreck_entity = world
                .resource::<EntityObjMap>()
                .get_entity(shipwreck_id)
                .expect("run-owned Shipwreck entity");
            let (mut torches, mut flint) = {
                let mut shipwreck_inventory = world
                    .get_mut::<Inventory>(shipwreck_entity)
                    .expect("Shipwreck inventory");
                let torch_index = shipwreck_inventory
                    .items
                    .iter()
                    .position(|item| item.name == "Crude Torch")
                    .expect("starter torch stack");
                let torches = shipwreck_inventory.items.remove(torch_index);
                let flint_index = shipwreck_inventory
                    .items
                    .iter()
                    .position(|item| item.class == crate::constants::IGNITION_TOOL)
                    .expect("starter Flint Shard");
                let flint = shipwreck_inventory.items.remove(flint_index);
                (torches, flint)
            };
            torches.owner = hero_id;
            flint.owner = hero_id;
            let ids = (torches.id, flint.id);
            let mut hero_inventory = world
                .get_mut::<Inventory>(hero_entity)
                .expect("hero inventory");
            hero_inventory.items.push(torches);
            hero_inventory.items.push(flint);
            ids
        };
        ...
        game.inject(PlayerEvent::Equip {
            player_id,
            obj_id: hero_id,
            item_id: torch_stack_id,
            status: true,
        });
        game.tick(2);

        let (first_lit_torch_id, first_flint_durability) = {
            let inventory = game
                .world()
                .get::<Inventory>(hero_entity)
                .expect("hero inventory after first ignition");
            let lit_torch = inventory
                .items
                .iter()
                .find(|item| item.class == crate::constants::TORCH && item.equipped)
                .expect("one equipped lit torch");
            assert_eq!(lit_torch.quantity, 1);
            assert!(lit_torch.image.starts_with("lit"));
            assert_eq!(
                inventory
                    .get_by_id(torch_stack_id)
                    .expect("remaining torch stack")
                    .quantity,
                2
            );
            (
                ...
                inventory
                    .get_by_id(flint_id)
                    .and_then(|item| item.durability)
                    .expect("Flint durability after first ignition"),
            )
        };
        assert_eq!(first_flint_durability, 19);

        // A duplicate authoritative request is a no-op: it must not consume
        // another Flint use or reset/split the already equipped torch.
        game.inject(PlayerEvent::Equip {
            player_id,
            obj_id: hero_id,
            item_id: first_lit_torch_id,
            status: true,
        });
        game.tick(2);
        assert_eq!(
            game.world()
                .get::<Inventory>(hero_entity)
                .expect("hero inventory after duplicate equip")
                .get_by_id(flint_id)
                .and_then(|item| item.durability),
            Some(first_flint_durability)
        );
        ...
        let buckler_id = {
            ...
            world
                .get_mut::<Inventory>(hero_entity)
                .expect("hero inventory before equipping buckler")
                .new(item_id, "Copper Buckler".to_string(), 1, &item_templates);
            item_id
        };
        game.inject(PlayerEvent::Equip {
            player_id,
        ...
        game.tick(2);
        {
            let mut inventory = game
                ...
                .world_mut()
                .get_mut::<Inventory>(hero_entity)
                .expect("hero inventory before final Flint use");
            assert!(inventory.get_by_id(first_lit_torch_id).is_none());
            assert!(inventory
                .get_by_id(buckler_id)
            ...
            assert_eq!(
                inventory
                    ...
                    .and_then(|item| item.durability),
                Some(first_flint_durability),
                "equipping another off-hand item must extinguish the torch without using Flint"
            );
            inventory
                .items
                .iter_mut()
                .find(|item| item.id == flint_id)
                .expect("Flint before final use")
                .durability = Some(1);
        }
        ...
        game.tick(2);
        let second_lit_torch_id = {
            let inventory = game
                .world()
                .get::<Inventory>(hero_entity)
                .expect("hero inventory after final Flint use");
            assert!(inventory.get_by_id(flint_id).is_none());
            inventory
                .items
                .iter()
                .find(|item| item.class == crate::constants::TORCH && item.equipped)
                .expect("torch lit by final Flint use")
                .id
        };
        ...
        assert!(packets.iter().any(|packet| matches!(
            ...
            ResponsePacket::Error { errmsg }
                if errmsg.contains("usable Ignition Tool")
        )));

        let inventory = game
            .world()
            .get::<Inventory>(hero_entity)
            .expect("hero inventory after rejected ignition");
        let remaining_torches: Vec<_> = inventory
            .items
            .iter()
            .filter(|item| item.class == crate::constants::TORCH)
            .collect();
        assert_eq!(remaining_torches.len(), 1);
        assert_eq!(remaining_torches[0].id, torch_stack_id);
        assert_eq!(remaining_torches[0].quantity, 1);
        assert!(!remaining_torches[0].equipped);
        assert_eq!(remaining_torches[0].image, "crudetorch");
    }
    ...
    fn revised_opening_shipwreck_is_accessible_only_to_its_run_owner() {
        ...
        game.start_packet_capture();
        game.inject(PlayerEvent::InfoItemTransfer {
            player_id: owner_id,
            source_id: owner_hero_id,
            target_id: shipwreck_id,
        });
        game.tick(3);
        let packets = game.finish_packet_capture();
        assert!(packets.iter().all(|packet| !matches!(
            packet,
            ResponsePacket::InfoItemTransfer { target_id, .. } if *target_id == shipwreck_id
        )));
        assert!(packets.iter().any(|packet| matches!(
            packet,
            ResponsePacket::Error { errmsg } if errmsg.contains("Search the Shipwreck")
        )));
        ...
        assert!(game.finish_packet_capture().iter().all(|packet| !matches!(
            packet,
            ResponsePacket::InfoPOI { id, .. } if *id == shipwreck_id
        )));
    ...
    #[test]
    fn one_hex_hero_visibility_blocks_gathering_and_building_with_notice() {
        let mut game = HeadlessGame::new(10_000);
        let player_id = game.spawn_hero("Warrior", "DarknessWorkGateBot");
        let (hero_id, hero_entity) = {
            let world = game.app.world_mut();
            let hero_id = world
                .resource::<Ids>()
            ...
            let hero_entity = world
                ...
                .expect("headless hero entity");
            world
                .get_mut::<Viewshed>(hero_entity)
                .expect("headless hero viewshed")
                .range = 1;
            (hero_id, hero_entity)
        };
        ...
        game.tick(3);
        let gather_packets = game.finish_packet_capture();
        assert!(gather_packets.iter().any(|packet| matches!(
            packet,
            ResponsePacket::Notice { noticemsg, .. }
                if noticemsg == crate::player::INSUFFICIENT_WORK_VISIBILITY_NOTICE
        )));
        assert_ne!(
            game.world()
                .get::<State>(hero_entity)
                .expect("hero state after rejected gather"),
            &State::Gathering
        );

        let dark_foundation_id = {
            let world = game.app.world_mut();
            let foundation_id = world.resource_mut::<Ids>().new_obj_id();
            let foundation_entity = world.spawn((Id(foundation_id), PlayerId(player_id))).id();
            world
                .resource_mut::<Ids>()
                .new_obj(foundation_id, player_id);
            world
                .resource_mut::<EntityObjMap>()
                .new_obj(foundation_id, foundation_entity);
            foundation_id
        };
        ...
        game.inject(PlayerEvent::Build {
            player_id,
            builder_id: hero_id,
            structure_id: dark_foundation_id,
        });
        game.tick(3);
        let start_build_packets = game.finish_packet_capture();
        assert!(start_build_packets.iter().any(|packet| matches!(
            packet,
            ResponsePacket::Notice { noticemsg, .. }
                if noticemsg == crate::player::INSUFFICIENT_WORK_VISIBILITY_NOTICE
        )));
        assert_ne!(
            game.world()
                .get::<State>(hero_entity)
                .expect("hero state after rejected build"),
            &State::Building
        );
        ...
        game.app
            .world_mut()
            .get_mut::<Viewshed>(hero_entity)
            .expect("headless hero viewshed before build")
            .range = 1;
        game.start_packet_capture();
        ...
        game.tick(3);
        let build_packets = game.finish_packet_capture();
        assert!(build_packets.iter().any(|packet| matches!(
            packet,
            ResponsePacket::Notice { noticemsg, .. }
                if noticemsg == crate::player::INSUFFICIENT_WORK_VISIBILITY_NOTICE
        )));
        assert!(build_packets.iter().all(|packet| !matches!(
            packet,
            ResponsePacket::CreateFoundation { result } if result == "success"
        )));
    }

    #[test]
    fn player_discovered_spring_refills_waterskin_without_global_reveal() {
        let mut game = HeadlessGame::new(10_000);
        let player_id = game.spawn_hero("Warrior", "DiscoveredSpringBot");
        let (hero_id, hero_entity, spring_pos, spring_name, empty_waterskin_id) = {
            let world = game.app.world_mut();
            let hero_id = world
                .resource::<Ids>()
                .get_hero(player_id)
            ...
            let spring_pos = {
                ...
                map.base
                    .iter()
                    .enumerate()
                    .map(|(index, _)| Map::index_to_pos(index))
                    .map(|pos| Position { x: pos.0, y: pos.1 })
                    .find(|pos| {
                        Map::is_passable(pos.x, pos.y, map)
                            && !Map::are_tile_types_nearby(
                                *pos,
                                vec![crate::map::TileType::River],
                                map,
                            )
                    })
                    .expect("non-river spring test tile")
            };
            *world
                .get_mut::<Position>(hero_entity)
                .expect("headless hero position") = spring_pos;
            ...
            world.resource_mut::<Objectives>().insert(
                player_id,
                PlayerObjectives {
            ...
            assert!(!Resource::is_valid_type(
                SPRING_WATER.to_string(),
                spring_pos,
                &world.resource::<Resources>(),
            ));
            assert!(!Resource::is_valid_type_for_player(
                SPRING_WATER.to_string(),
                spring_pos,
                &world.resource::<Resources>(),
                &world.resource::<ResourceDiscoveries>(),
                player_id,
            ));
        ...
        game.tick(4);
        let failed_packets = game.finish_packet_capture();
        assert!(failed_packets.iter().any(|packet| matches!(
            packet,
            ResponsePacket::Error { errmsg } if errmsg.contains("spring water resource")
        )));
        assert!(
            !game
                .world()
            ...
            "a failed fill attempt must not complete the tutorial lesson"
        );
        assert_eq!(
            game.world()
                .get::<Inventory>(hero_entity)
                .expect("hero inventory after failed spring refill")
                .items
                .iter()
                .filter(|item| item.name == crate::constants::WATERSKIN_EMPTY)
                .map(|item| item.quantity)
                .sum::<i32>(),
            1
        );
        ...
        game.app
            .world_mut()
            .resource_mut::<ResourceDiscoveries>()
            .discover(player_id, spring_pos, spring_name);
        ...
        let inventory = game
            .world()
            .get::<Inventory>(hero_entity)
            .expect("hero inventory after spring refill");
        assert_eq!(
            inventory
                .items
                .iter()
        ...
        assert_eq!(
            inventory
                .items
                .iter()
                .filter(|item| item.name == crate::constants::WATERSKIN_FILLED)
                .map(|item| item.quantity)
                .sum::<i32>(),
            1
        );
        assert!(
            game.world()
                .resource::<Objectives>()
                .get(&player_id)
                .expect("objectives after successful spring refill")
                .fill_empty_waterskin,
            "the successful Empty-to-Filled Waterskin conversion completes the lesson"
        );
        assert_eq!(
            *game
                .world()
                .get::<Position>(hero_entity)
                .expect("hero remains on spring tile"),
            spring_pos
        );
        assert!(packets.iter().all(|packet| !matches!(
            packet,
            ResponsePacket::Error { errmsg }
                if errmsg.contains("spring water resource")
        )));
    }

    #[test]
    fn well_foundation_requires_a_player_discovered_spring() {
        let mut game = HeadlessGame::new(10_000);
        let player_id = game.spawn_hero("Warrior", "WellPlacementBot");
        let (hero_id, spring_pos, spring_name) = {
            let hero = game.observe().hero.expect("headless hero");
            let spring_pos = passable_unoccupied_adjacent_position(&mut game, hero.pos);
            game.move_hero_for_test(spring_pos);
            ...
            let spring_name = "Well Test Spring Water".to_string();
            Resource::create(
                spring_name.clone(),
                SPRING_WATER.to_string(),
                "moonlitspringwater".to_string(),
            ...
            (hero_id, spring_pos, spring_name)
        };
        ...
        game.inject(PlayerEvent::CreateFoundation {
            player_id,
            source_id: hero_id,
            structure_name: crate::structure::WELL.to_string(),
        });
        game.tick(3);
        let hidden_packets = game.finish_packet_capture();
        assert!(hidden_packets.iter().any(|packet| matches!(
            packet,
            ResponsePacket::Error { errmsg }
                if errmsg == "A Well must be built on a revealed Spring Water resource."
        )));
        let hidden_well_count = {
            let world = game.app.world_mut();
            let mut wells = world.query_filtered::<(&PlayerId, &Template), With<ClassStructure>>();
            wells
                .iter(world)
                .filter(|(owner, template)| {
                    owner.0 == player_id && template.0 == crate::structure::WELL
                })
                .count()
        };
        assert_eq!(hidden_well_count, 0);
        ...
        game.tick(3);
        let revealed_packets = game.finish_packet_capture();
        assert!(revealed_packets.iter().any(|packet| matches!(
            packet,
            ResponsePacket::CreateFoundation { result } if result == "success"
        )));
        let founded_well_count = {
            let world = game.app.world_mut();
            let mut wells = world
                .query_filtered::<(&PlayerId, &Template, &Position, &State), With<ClassStructure>>(
                );
            wells
                .iter(world)
                .filter(|(owner, template, pos, state)| {
                    owner.0 == player_id
                        && template.0 == crate::structure::WELL
                        && **pos == spring_pos
                        && **state == State::Founded
                })
                .count()
        };
        assert_eq!(founded_well_count, 1);
    }

    #[test]
    fn shipwreck_loot_all_is_rejected_without_transferring_items() {
        let mut game = HeadlessGame::new(10_000);
        let player_id = game.spawn_hero("Warrior", "ShipwreckLootAllBot");
        ...
        let hero_id = game
            ...
            .get_hero(player_id)
            .expect("headless hero id");
        let initial_shipwreck_item_count = {
            let world = game.app.world_mut();
            let shipwreck_entity = world
                .resource::<EntityObjMap>()
                .get_entity(shipwreck_id)
        ...
        assert!(initial_shipwreck_item_count > 1);

        game.start_packet_capture();
        game.inject(PlayerEvent::LootAll {
            player_id,
            source_id: shipwreck_id,
            target_id: hero_id,
        });
        ...
        assert!(packets.iter().any(|packet| matches!(
            packet,
            ResponsePacket::Error { errmsg }
                if errmsg == "Loot All is only available for enemy corpses and dropped bags."
        )));
        assert!(!packets
            .iter()
            .any(|packet| matches!(packet, ResponsePacket::ItemTransfer { .. })));

        let remaining_shipwreck_item_count = {
            let world = game.app.world_mut();
            let shipwreck_entity = world
                .resource::<EntityObjMap>()
                .get_entity(shipwreck_id)
        ...
        assert_eq!(remaining_shipwreck_item_count, initial_shipwreck_item_count);
    }

    #[test]
    fn dropped_hero_item_creates_one_public_bag_that_another_player_can_loot() {
        use crate::constants::{
            DROPPED_BAG_LIFETIME_TICKS, DROPPED_BAG_TEN_SECOND_WARNING_TICKS,
            LOOT_POI_EMPTY_DESPAWN_TICKS, NPC_PLAYER_ID,
        };

        let mut game = HeadlessGame::new(10_000);
        let owner_player_id = game.spawn_hero("Warrior", "DroppedBagOwnerBot");
        let (owner_hero_id, owner_hero_entity, owner_pos, dropped_item_id, refill_item_id) = {
            let world = game.app.world_mut();
            let owner_hero_id = world
                .resource::<Ids>()
                .get_hero(owner_player_id)
                .expect("owner hero id");
            let owner_hero_entity = world
                .resource::<EntityObjMap>()
                .get_entity(owner_hero_id)
            ...
            let owner_pos = *world
                .get::<Position>(owner_hero_entity)
                .expect("owner hero position");
            let dropped_item_id = world
                .get::<Inventory>(owner_hero_entity)
                .expect("owner hero inventory")
                .items
                .iter()
                .find(|item| item.name == "Sharpened Stick")
                .expect("starting Sharpened Stick")
                .id;
            let refill_item_id = world
                .get::<Inventory>(owner_hero_entity)
                .expect("owner hero inventory")
                .items
                .iter()
                .find(|item| item.name == "Tattered Shirt")
                .expect("starting Tattered Shirt")
                .id;
            (
                owner_hero_id,
                owner_hero_entity,
        ...
        game.inject(PlayerEvent::DropItem {
            player_id: owner_player_id,
            item_id: dropped_item_id,
        });
        game.tick(3);

        let (bag_id, bag_entity, original_expires_at) = {
            let world = game.app.world_mut();
            assert!(world
                .get::<Inventory>(owner_hero_entity)
                .expect("owner hero inventory after drop")
                .get_by_id(dropped_item_id)
                .is_none());
            let mut bags = world.query::<(Entity, &Id, &PlayerId, &Position, &Template)>();
            let matching_bags = bags
                .iter(world)
                .filter(|(_, _, _, pos, template)| {
                    **pos == owner_pos && template.0 == templates::DROPPED_BAG_TEMPLATE
                })
                .map(|(entity, id, player_id, _, _)| (entity, id.0, player_id.0))
                .collect::<Vec<_>>();
            assert_eq!(matching_bags.len(), 1, "one bag should represent this hex");
            assert_eq!(matching_bags[0].2, NPC_PLAYER_ID, "the bag is public");
            let (bag_entity, bag_id, _) = matching_bags[0];
            let dropped_item = world
                .get::<Inventory>(bag_entity)
                .expect("dropped bag inventory")
                .get_by_id(dropped_item_id)
                .expect("dropped item in bag");
            assert_eq!(dropped_item.owner, bag_id);
            assert!(!dropped_item.equipped);
            assert!(
                world.get::<crate::obj::Storage>(bag_entity).is_none(),
                "villager jobs must not treat dropped bags as settlement storage"
            );
            let original_expires_at = world
                .get::<DroppedBag>(bag_entity)
                .expect("dropped bag lifetime")
                .expires_at;
            let remaining_lifetime = original_expires_at - world.resource::<GameTick>().0;
            assert!(remaining_lifetime <= DROPPED_BAG_LIFETIME_TICKS);
            assert!(remaining_lifetime >= DROPPED_BAG_LIFETIME_TICKS - 3);
            assert!(world.resource::<GameEvents>().values().any(|event| {
                event.run_tick == original_expires_at
                    && matches!(
                        event.event_type,
                        GameEventType::DespawnObj { obj_id } if obj_id == bag_id
                    )
            }));
            (bag_id, bag_entity, original_expires_at)
        };

        // Preserve the production-duration assertion above, then shorten this
        // instance so the test can prove the fixed deadline without pumping
        // five real-time minutes' worth of server frames.
        let shortened_expires_at = game.game_tick() + LOOT_POI_EMPTY_DESPAWN_TICKS + 300;
        {
            let world = game.app.world_mut();
            world
                .get_mut::<DroppedBag>(bag_entity)
                .expect("dropped bag lifetime")
                .expires_at = shortened_expires_at;
            let mut game_events = world.resource_mut::<GameEvents>();
            let fixed_event = game_events
                .values_mut()
                .find(|event| {
                    event.run_tick == original_expires_at
                        && matches!(
                ...
                .expect("fixed dropped bag despawn event");
            fixed_event.run_tick = shortened_expires_at;
        }
        ...
        assert!(game.finish_packet_capture().iter().any(|packet| matches!(
            packet,
            ResponsePacket::Notice { noticemsg, .. } if noticemsg.contains("disappears in one minute")
        )));

        let looter_player_id = spawn_connected_helper(&mut game, "DroppedBagLooterBot");
        let looter_hero_id = {
            let world = game.app.world_mut();
            let looter_hero_id = world
                .resource::<Ids>()
                .get_hero(looter_player_id)
                .expect("looter hero id");
            let looter_hero_entity = world
                .resource::<EntityObjMap>()
                .get_entity(looter_hero_id)
                .expect("looter hero entity");
            *world
                .get_mut::<Position>(looter_hero_entity)
                .expect("looter hero position") = owner_pos;
            looter_hero_id
        };

        game.inject(PlayerEvent::LootAll {
            player_id: looter_player_id,
            source_id: bag_id,
            target_id: looter_hero_id,
        });
        game.tick(3);

        let looter_hero_entity = game
            .world()
            .resource::<EntityObjMap>()
            .get_entity(looter_hero_id)
            .expect("looter hero entity after loot");
        let looted_item = game
            .world()
            .get::<Inventory>(looter_hero_entity)
            .expect("looter hero inventory")
            .get_by_id(dropped_item_id)
            .expect("other player looted dropped item");
        assert_eq!(looted_item.owner, looter_hero_id);
        assert!(game
            .world()
            .get::<Inventory>(bag_entity)
            .expect("emptied bag before cleanup")
            .items
            .is_empty());
        assert!(game.world().resource::<GameEvents>().values().any(|event| {
            event.run_tick < shortened_expires_at
                && matches!(
                    event.event_type,
                    GameEventType::DespawnObj { obj_id } if obj_id == bag_id
                )
        ...
        game.inject(PlayerEvent::DropItem {
            player_id: owner_player_id,
            item_id: refill_item_id,
        });
        game.tick(3);
        assert_eq!(
            game.world()
                .get::<DroppedBag>(bag_entity)
                .expect("refilled bag lifetime")
                .expires_at,
            shortened_expires_at,
            "adding an item must not renew the bag lifetime"
        );
        assert_eq!(
            game.world()
                .resource::<GameEvents>()
                ...
                .count(),
            1,
            "refilling cancels only the empty cleanup and preserves fixed expiry"
        );
        game.tick((LOOT_POI_EMPTY_DESPAWN_TICKS + 3) as u32);
        assert_eq!(
            game.world()
                .get::<Inventory>(bag_entity)
                .expect("refilled bag survives its cancelled empty cleanup")
                .get_by_id(refill_item_id)
                .expect("second dropped item")
                .owner,
            bag_id
        );

        let ticks_until_ten_second_warning =
            shortened_expires_at - game.game_tick() - DROPPED_BAG_TEN_SECOND_WARNING_TICKS + 2;
        game.start_packet_capture();
        game.tick(ticks_until_ten_second_warning.max(2) as u32);
        assert!(game.finish_packet_capture().iter().any(|packet| matches!(
            packet,
            ResponsePacket::Notice { noticemsg, .. } if noticemsg.contains("disappears in 10 seconds")
        )));

        let ticks_until_fixed_expiry = shortened_expires_at - game.game_tick();
        game.tick((ticks_until_fixed_expiry + 3).max(3) as u32);
        assert!(game
            .world()
            .resource::<EntityObjMap>()
            .get_entity(bag_id)
    ...
    #[test]
    fn revised_opening_rat_ambush_interrupts_first_search_and_requires_retry() {
        use crate::game::{InitialEncounterState, OPENING_RAT_AMBUSH_DELAY_TICKS};
        ...
        assert!(!no_search_entry.opening_rat_ambush_armed);
        ...
        let mut game = HeadlessGame::new(10_000);
        let player_id = game.spawn_hero("Warrior", "OpeningAmbushBot");
        ...
        let hero_id = game
            ...
            .get_hero(player_id)
            .expect("opening hero id");
        let investigation_due_tick = game
            .world()
            .resource::<MapEvents>()
            .values()
            .find_map(|event| {
            ...
            .expect("first Shipwreck investigation event");
        let armed_entry = game
            .world()
            .resource::<InitialEncounterState>()
            .get(&player_id)
            .expect("armed opening encounter")
            .clone();
        assert!(armed_entry.opening_rat_ambush_armed);
        assert_eq!(
            armed_entry.opening_rat_spawn_tick,
            investigation_due_tick - crate::constants::INVESTIGATE_TICKS
                + OPENING_RAT_AMBUSH_DELAY_TICKS,
            "opening rat wave must be scheduled from the start of the first search"
        );
        assert!(
            armed_entry.opening_rat_spawn_tick < original_deadline,
            "the old 90-second placeholder must not delay an immediate ambush"
        );

        run_intro_check_at_or_after(&mut game, armed_entry.opening_rat_spawn_tick);
        let after_ambush = game
            .world()
            .resource::<InitialEncounterState>()
            .get(&player_id)
            .expect("opening encounter after ambush")
            .clone();
        assert!(after_ambush
            .opening_enemy_spawned
            .iter()
        ...
        assert!(!game
            ...
            .unwrap_or(false));
        assert_eq!(
            owned_living_villager_count(&mut game, player_id),
            0,
            "the interrupted first investigation must not rescue the villager"
        );
        assert!(game
            .world()
            .resource::<MapEvents>()
            ...
            .all(|event| !matches!(
                event.event_type,
                VisibleEvent::InvestigateEvent { target_id }
                    if event.obj_id == hero_id && target_id == shipwreck_id
            )));
        let hero_entity = game
            .world()
            .resource::<EntityObjMap>()
        ...
        assert_eq!(
            *game
                .world()
                .get::<State>(hero_entity)
                .expect("hero state after ambush"),
            State::None
        );
        assert!(
            game.world().get::<ActionProgress>(hero_entity).is_none(),
            "the interrupted investigation must clear its progress indicator"
        );
        let ambush_packets = game.finish_packet_capture();
        assert!(ambush_packets.iter().any(|packet| matches!(
            packet,
            ResponsePacket::Notice { noticemsg, .. }
                if noticemsg == "Giant rats emerge! Investigation interrupted."
        )));
        ...
        let defeated_at = game.game_tick();
        mark_obj_ids_dead(&mut game, &after_ambush.rat_ids, defeated_at);
        run_intro_check_at_or_after(&mut game, defeated_at + 1);
        ...
        game.tick((crate::constants::INVESTIGATE_TICKS + 5) as u32);

        assert!(
            game.world()
                .resource::<Objectives>()
                .get(&player_id)
                .expect("objectives after repeated search")
                .scavenge_shipwreck
        );
        assert!(game
            .world()
            .resource::<InvestigatedPOIs>()
            .get(&player_id)
            .is_some_and(|pois| pois.contains(&shipwreck_id)));
        let after_retry = game
            .world()
            .resource::<InitialEncounterState>()
            .get(&player_id)
            .expect("opening encounter after repeated search");
        assert_eq!(after_retry.rat_ids, after_ambush.rat_ids);
        assert!(after_retry
            .opening_enemy_spawned
            .iter()
            .all(|spawned| *spawned));
        assert!(after_retry.villager_event_scheduled);
        assert_eq!(
            owned_living_villager_count(&mut game, player_id),
            1,
            "the successfully completed retry should immediately rescue one villager"
        );
        let (rescued_hero_pos, rescued_villager_pos, rescued_campfire_pos) = {
            use crate::obj::Campfire;

            let world = game.app.world_mut();
            let mut hero_query =
                world.query_filtered::<(&PlayerId, &Position), With<SubclassHero>>();
            let rescued_hero_pos = hero_query
                .iter(world)
                .find(|(owner, _)| owner.0 == player_id)
                .map(|(_, pos)| *pos)
                .expect("rescued villager owner hero");
            let mut villager_query =
                world.query_filtered::<(&PlayerId, &Position), With<SubclassVillager>>();
            let rescued_villager_pos = villager_query
                .iter(world)
                .find(|(owner, _)| owner.0 == player_id)
                .map(|(_, pos)| *pos)
                .expect("rescued Shipwreck villager");
            let mut campfire_query =
                world.query_filtered::<(&PlayerId, &Position), With<Campfire>>();
            let rescued_campfire_pos = campfire_query
                .iter(world)
                .find(|(owner, _)| owner.0 == player_id)
                .map(|(_, pos)| *pos)
                .expect("rescued villager owner's Campfire");
            (rescued_hero_pos, rescued_villager_pos, rescued_campfire_pos)
        };
        assert_ne!(
            rescued_villager_pos, rescued_hero_pos,
            "the rescued villager must never spawn on top of the hero"
        );
        assert!(
            Map::dist(rescued_villager_pos, rescued_campfire_pos)
                <= crate::game::RESCUED_VILLAGER_SPAWN_SEARCH_RADIUS as u32,
            "the rescued villager should remain near the starting camp"
        );
    }

    #[test]
    fn revised_opening_successful_search_rescues_once_without_burrow_or_rat_defeat() {
        use crate::game::{InitialEncounterState, PlayerIntroState};

        let mut game = HeadlessGame::new(10_000);
        let player_id = game.spawn_hero("Warrior", "ImmediateShipwreckRescueBot");
        let shipwreck_id = investigate_shipwreck_for_smoke(&mut game, player_id);

        let opening_entry = game
            .world()
            .resource::<InitialEncounterState>()
            .get(&player_id)
            .expect("opening encounter after successful Shipwreck investigation")
            .clone();
        assert!(opening_entry.villager_event_scheduled);
        assert!(opening_entry
            .opening_enemy_spawned
            .iter()
            .all(|spawned| !*spawned));
        assert!(opening_entry
        ...
        assert!(
            game.world()
                ...
                .get(&player_id)
                .expect("intro state after successful Shipwreck investigation")
                .villager_spawned
        );
        assert_eq!(owned_living_villager_count(&mut game, player_id), 1);
        ...
        let has_burrow = {
            ...
            structures
                .iter(world)
                .any(|(owner, template)| owner.0 == player_id && template.0 == "Burrow")
        };
        assert!(!has_burrow, "the rescue must not wait for a Burrow");
        assert!(!game
            .world()
            .resource::<crate::structure::Plans>()
            .contains(player_id, "Watchtower"));
        assert!(!game
            .world()
            .resource::<crate::structure::Plans>()
            .contains(player_id, "Lumbercamp"));
        {
            let world = game.app.world_mut();
            let mut merchant_inventory_query = world.query_filtered::<&Inventory, With<Merchant>>();
            let merchant_inventory = merchant_inventory_query
                .iter(world)
                .next()
                .expect("the run's traveling merchant inventory");
            assert!(merchant_inventory.items.iter().any(|item| {
                item.name == "Watchtower Deed"
                    && item.class == crate::item::DEED
                    && item.subclass == "Watchtower"
                    && item.quantity == 1
            }));
            assert_eq!(
                world
                    .resource::<crate::trade::Prices>()
                    .get_sell_price("Watchtower Deed".to_string()),
                Some(100),
                "the merchant must have a valid sale price for the deed"
            );
        }
        {
            let world = game.app.world_mut();
            let hero_id = world
                .resource::<Ids>()
                .get_hero(player_id)
                .expect("rescued villager owner hero id");
            let hero_entity = world
                .resource::<EntityObjMap>()
                .get_entity(hero_id)
                .expect("rescued villager owner hero entity");
            let inventory = world
                .get::<Inventory>(hero_entity)
                .expect("rescued villager owner inventory");
            assert!(inventory
                .items
                .iter()
                .any(|item| item.name == "Burrow Deed" && item.quantity == 1));
            assert!(!inventory
                .items
                .iter()
                .any(|item| item.name == "Lumbercamp Deed"));
        }
        ...
        game.tick((crate::constants::INVESTIGATE_TICKS + 5) as u32);
        let reconciliation_tick = game.game_tick() + 10;
        run_intro_check_at_or_after(&mut game, reconciliation_tick);
        assert_eq!(
            owned_living_villager_count(&mut game, player_id),
            1,
            "repeated searches and reconciliation ticks must not duplicate the rescued villager"
        );

        game.disconnect_player();
        game.tick(3);
        game.reconnect_player_with_login();
        let post_reconnect_tick = game.game_tick() + 10;
        run_intro_check_at_or_after(&mut game, post_reconnect_tick);
        assert_eq!(
            owned_living_villager_count(&mut game, player_id),
            1,
            "reconnect reconciliation must preserve the one-shot rescue"
        );
        assert_eq!(
            hero_item_quantity(&game, player_id, "Burrow Deed"),
            1,
            "reconnect reconciliation must not duplicate the rescue deed"
        );
    }

    #[test]
    fn rescued_villager_grants_each_settlement_deed_once_at_its_milestone() {
        let mut game = HeadlessGame::new(10_000);
        let player_id = game.spawn_hero("Warrior", "VillagerDeedMilestoneBot");
        investigate_shipwreck_for_smoke(&mut game, player_id);

        for structure_name in ["Burrow", "Lumbercamp", "Shelter Tent", "Stockade"] {
            assert!(
                !game
                    .world()
                    .resource::<crate::structure::Plans>()
                    .contains(player_id, structure_name),
                "{structure_name} must not remain a starter plan"
            );
        }
        assert_eq!(hero_item_quantity(&game, player_id, "Burrow Deed"), 1);
        assert_eq!(hero_item_quantity(&game, player_id, "Lumbercamp Deed"), 0);

        let hero_id = game
            .world()
            .resource::<Ids>()
            .get_hero(player_id)
            .expect("deed owner hero id");
        game.start_packet_capture();
        game.inject(PlayerEvent::CreateFoundation {
            player_id,
            source_id: hero_id,
        ...
        assert!(game.finish_packet_capture().iter().any(|packet| matches!(
            packet,
            ResponsePacket::Error { errmsg }
                if errmsg == "You have not learned how to build a Stockade. Use its deed first."
        )));
        ...
        let burrow_deed_id = {
            ...
            let hero_entity = world
                .resource::<EntityObjMap>()
                .get_entity(hero_id)
                .expect("deed owner hero entity");
            world
                .get::<Inventory>(hero_entity)
                .expect("deed owner inventory")
                .items
                .iter()
                .find(|item| item.name == "Burrow Deed")
                .expect("rescued villager Burrow Deed")
                .id
        };
        game.inject(PlayerEvent::Use {
            player_id,
            obj_id: hero_id,
            item_id: burrow_deed_id,
        });
        game.tick(3);
        assert!(game
            .world()
            .resource::<crate::structure::Plans>()
            .contains(player_id, "Burrow"));
        assert_eq!(hero_item_quantity(&game, player_id, "Burrow Deed"), 0);

        let (villager_entity, villager_pos) = {
            let world = game.app.world_mut();
            let mut villagers = world.query_filtered::<
                (Entity, &PlayerId, &Position),
        ...
        // Lumbercamp deed. The Shelter Tent waits for a later tired transition.
        {
            ...
            world
                ...
                .expect("rescued villager tiredness")
                .tired = 65.0;
            world.entity_mut(villager_entity).insert(Order::Gather {
                res_type: crate::constants::LOG.to_string(),
                pos: villager_pos,
                storage_pos: None,
                storage_id: None,
            });
        ...
        game.tick(1);
        assert_eq!(hero_item_quantity(&game, player_id, "Lumbercamp Deed"), 1);
        assert_eq!(
        ...
        let campfire_id = {
            ...
            structures
                .iter(world)
                .find(|(_, owner, template)| owner.0 == player_id && template.0 == "Campfire")
                .map(|(id, ..)| id.0)
                .expect("starting Campfire")
        };
        game.start_packet_capture();
        game.inject(PlayerEvent::InfoUpgrade {
            player_id,
            structure_id: campfire_id,
        });
        game.tick(3);
        ...
        game.inject(PlayerEvent::StartUpgrade {
            player_id,
            structure_id: campfire_id,
            selected_upgrade: "Shelter Tent".to_string(),
        });
        game.tick(3);
        assert!(game.finish_packet_capture().iter().any(|packet| matches!(
            packet,
        ...
        let shelter_deed_id = {
            ...
            game.world()
                .get::<Inventory>(hero_entity)
                .expect("Shelter Tent deed owner inventory")
                .items
                .iter()
        ...
        game.inject(PlayerEvent::Use {
            player_id,
            obj_id: hero_id,
            item_id: shelter_deed_id,
        });
        game.tick(3);
        assert!(game
            .world()
            .resource::<crate::structure::Plans>()
            .contains(player_id, "Shelter Tent"));
        game.start_packet_capture();
        game.inject(PlayerEvent::InfoUpgrade {
            player_id,
            structure_id: campfire_id,
        ...
        assert!(game.finish_packet_capture().iter().any(|packet| matches!(
            ...
            ResponsePacket::InfoUpgrade { id, upgrade_list }
                if *id == campfire_id
                    && upgrade_list.iter().any(|upgrade| upgrade.name == "Shelter Tent")
        )));

        // Day 5 is personal survival time, not the global world's calendar.
        let current_tick = game.game_tick();
        game.app
            .world_mut()
            .resource_mut::<PlayerIntroState>()
            .get_mut(&player_id)
            .expect("villager deed intro state")
            .start_tick = current_tick - (3 * GAME_TICKS_PER_DAY);
        game.tick(1);
        assert_eq!(hero_item_quantity(&game, player_id, "Stockade Deed"), 0);
        ...
        game.app
            ...
            .get_mut(&player_id)
            .expect("villager deed intro state")
            .start_tick = current_tick - (4 * GAME_TICKS_PER_DAY);
        game.tick(3);
        assert_eq!(
            hero_item_quantity(&game, player_id, "Stockade Deed"),
            0,
            "the villager should wait to deliver the Day-5 deed until the player reconnects"
        );
        game.reconnect_player_with_login();
        game.tick(3);
        assert_eq!(hero_item_quantity(&game, player_id, "Stockade Deed"), 1);

        game.tick(20);
        assert_eq!(hero_item_quantity(&game, player_id, "Lumbercamp Deed"), 1);
        assert_eq!(hero_item_quantity(&game, player_id, "Shelter Tent Deed"), 0);
        assert_eq!(hero_item_quantity(&game, player_id, "Stockade Deed"), 1);
    }
    ...
    fn revised_opening_reconnect_and_duplicate_new_player_do_not_duplicate_spawns() {
        ...
        assert_eq!(original.hero_items.len(), 3);
        assert_eq!(
            original
                .structure_ids
                .iter()
    ...
    fn revised_opening_salvages_and_builds_a_normal_burrow_without_gathering() {
        ...
        let burrow_deed_id = {
            let hero_entity = game
                ...
                .resource::<EntityObjMap>()
                .get_entity(hero_id)
                .expect("opening hero entity for Burrow deed");
            game.world()
                .get::<Inventory>(hero_entity)
                .expect("opening hero inventory for Burrow deed")
                .items
                .iter()
                .find(|item| item.name == "Burrow Deed")
                .expect("rescued villager Burrow deed")
    ...
    #[test]
    fn shipwreck_log_stack_partially_transfers_when_hero_has_47_capacity_left() {
        let mut game = HeadlessGame::new(2_000);
        let player_id = game.spawn_hero("Warrior", "OpeningPartialLogTransferBot");
        let shipwreck_id = investigate_shipwreck_for_smoke(&mut game, player_id);

        let (hero_id, shipwreck_log_id) = {
            let world = game.app.world_mut();
            let hero_id = world
                .resource::<Ids>()
                .get_hero(player_id)
            ...
            let mut hero_inventory = world
                .get_mut::<Inventory>(hero_entity)
                .expect("opening hero inventory");
            assert_eq!(hero_inventory.get_total_weight(), 13);
            hero_inventory.items.push(crate::item::Item {
                id: -100,
                owner: hero_id,
                name: "Opening test ballast".to_string(),
                quantity: 1,
                durability: None,
            ...
            assert_eq!(hero_inventory.get_total_weight(), 53);

            let shipwreck_entity = world
                .resource::<EntityObjMap>()
                .get_entity(shipwreck_id)
                .expect("opening Shipwreck entity");
            let shipwreck_log_id = world
                .get::<Inventory>(shipwreck_entity)
                .expect("opening Shipwreck inventory")
                .items
                .iter()
                ...
                .expect("five starter Logs")
                .id;
            (hero_id, shipwreck_log_id)
        };

        game.inject(PlayerEvent::ItemTransfer {
            player_id,
            item_id: shipwreck_log_id,
            source_id: shipwreck_id,
            target_id: hero_id,
        });
        game.tick(3);
        ...
        let hero_entity = world
            .resource::<EntityObjMap>()
            .get_entity(hero_id)
            .expect("opening hero entity after transfer");
        let shipwreck_entity = world
            .resource::<EntityObjMap>()
            .get_entity(shipwreck_id)
            .expect("opening Shipwreck entity after transfer");
        let hero_inventory = world
            .get::<Inventory>(hero_entity)
            .expect("opening hero inventory after transfer");
        let shipwreck_inventory = world
            .get::<Inventory>(shipwreck_entity)
            .expect("opening Shipwreck inventory after transfer");

        assert_eq!(hero_inventory.get_total_weight(), 93);
        assert_eq!(
            hero_inventory
                .items
                .iter()
                .filter(|item| item.name == "Springbranch Maple Log")
                .map(|item| item.quantity)
                .sum::<i32>(),
            4
        );
    ...
    #[test]
    fn personal_crisis_mode_uses_the_bounded_minor_skirmish_track() {
        use crate::event::{GameEventType, GameEvents};
        use crate::game::{
            InitialEncounterState, IntroEncounterState, PlayerIntroState, PreCrisisSkirmishSlot,
        ...
        assert_eq!(
            ...
            None,
            "the historical boar/crab chain must not also run in PersonalCrisis mode"
        );

        let skirmish_due = game.game_tick() + 410;
        run_intro_check_at_or_after(&mut game, skirmish_due);
        let (skirmish_id, skirmish_pos, skirmish_stats) = {
            let world = game.app.world_mut();
            let mut query = world.query::<(
                &Id,
                &Position,
            ...
            query
                .iter(world)
                .find(|(_, _, _, skirmish, dead)| {
                    skirmish.owner_player_id == player_id
                        && skirmish.slot == PreCrisisSkirmishSlot::WildernessNuisance
                        && dead.is_none()
                })
                .map(|(id, pos, stats, _, _)| (id.0, *pos, stats.clone()))
                .expect("the first bounded wilderness nuisance should spawn")
        };
        let sanctuary_zones = game.world().resource::<crate::game::SanctuaryZones>();
        assert!(!sanctuary_zones.contains(skirmish_pos));
        assert_eq!(skirmish_stats.hp, 8);
        assert_eq!(skirmish_stats.base_damage, Some(1));
        assert!(game
            .world()
            .resource::<RunSpawnedObjs>()
            .contains_for_player(player_id, skirmish_id));
        assert!(
            game.world()
        ...
        let necromancer_pending_or_revealed = game
            .world()
            .resource::<GameEvents>()
            .values()
            ...
            || game
                .world()
                .resource::<EntityObjMap>()
                .get_entity(entry.necromancer_id)
                .and_then(|entity| game.world().get::<State>(entity))
                .is_some_and(|state| *state != State::Hiding);
        assert!(
            necromancer_pending_or_revealed,
            "the rescue-anchored Necromancer event should be pending or already revealed"
        );

        // This regression isolates the bounded nuisance sequence. Prevent the
        // separate Necromancer introduction from legitimately occupying the
        // one-hostile-at-a-time safety gate while the clock is advanced.
        game.app
            .world_mut()
            .resource_mut::<GameEvents>()
            .retain(|_, event| !matches!(event.event_type, GameEventType::NecroEvent { .. }));
        if let Some(necromancer_entity) = game
            .world()
            .resource::<EntityObjMap>()
            .get_entity(entry.necromancer_id)
        {
            let dead_at = game.game_tick();
            game.app.world_mut().entity_mut(necromancer_entity).insert((
                State::Dead,
                StateDead {
                    dead_at,
                    killer: "Headless bounded-skirmish isolation".to_string(),
                },
            ));
        }

        // Exercise the complete minimum-length sequence. The randomized target
        // has separate distribution coverage; fixing this run at three keeps
        // the headless regression below the unrelated Necromancer deadline.
        crate::game::set_pre_fortification_encounter_target_for_test(
            game.app.world_mut(),
            player_id,
            3,
        );
        spawn_explicit_completed_structure(&mut game, player_id, "Lumbercamp");
        let first_skirmish_dead_at = game.game_tick();
        mark_obj_ids_dead(&mut game, &[skirmish_id], first_skirmish_dead_at);

        let mut pre_fortification_waves = 1;
        for _ in 0..2 {
            let wave_due = game.game_tick() + (62 * crate::constants::TICKS_PER_SEC);
            run_intro_check_at_or_after(&mut game, wave_due);
            let scavengers = living_pre_crisis_units(
                &mut game,
                player_id,
                PreCrisisSkirmishSlot::WorksiteScavengers,
            );
            let progress = crate::game::pre_crisis_progress_for_test(game.world(), player_id);
            assert!(
                (1..=2).contains(&scavengers.len()),
                "each Lumbercamp nuisance wave should contain one or two easy creatures; progress={progress:?}"
            );
            let ids = scavengers
                .iter()
                ...
                .collect::<Vec<_>>();
            let dead_at = game.game_tick();
            mark_obj_ids_dead(&mut game, &ids, dead_at);
            pre_fortification_waves += 1;
        }
        assert_eq!(pre_fortification_waves, 3);

        let stockade_pos = spawn_explicit_completed_structure(&mut game, player_id, "Stockade");
        let scout_due = game.game_tick() + (22 * crate::constants::TICKS_PER_SEC);
        run_intro_check_at_or_after(&mut game, scout_due);
        let scouts =
            living_pre_crisis_units(&mut game, player_id, PreCrisisSkirmishSlot::GoblinScout);
        let scout_progress = crate::game::pre_crisis_progress_for_test(game.world(), player_id);
        assert!(
            (1..=2).contains(&scouts.len()),
            "completing the first Stockade should arm one or two inspecting Goblin Scouts; progress={scout_progress:?} stockade={stockade_pos:?}"
        );
        let sanctuary_zones = game.world().resource::<crate::game::SanctuaryZones>();
        assert!(scouts.iter().all(|(_, pos)| {
            (4..=7).contains(&Map::dist(stockade_pos, *pos)) && !sanctuary_zones.contains(*pos)
        }));
        assert_eq!(
            game.world()
                .resource::<crate::game::SettlementCrisisState>()
                .get(&player_id)
                .map(|crisis| crisis.phase),
            Some(crate::game::CrisisPhase::Dormant),
            "the fortification inspection must not require or advance Goblin pressure"
        );
    ...
    fn core_gameplay_checkpoint1_smoke_opening_chain_records_history_across_corpse_cleanup() {
        ...
        let mut game =
            HeadlessGame::new_with_director(10_000, crate::game::SurvivalDirectorMode::Legacy);
        ...
        let original = game
            ...
            .resource::<InitialEncounterState>()
            .get(&player_id)
            .expect("initial encounter state after arming the Shipwreck ambush")
            .clone();
        assert!(original.opening_rat_ambush_armed);
        {
            let mut encounters = game.app.world_mut().resource_mut::<InitialEncounterState>();
    ...
    fn core_gameplay_checkpoint1_smoke_early_settlement_uses_existing_economy() {
        ...
        let burrow_pos = spawn_explicit_completed_burrow(&mut game, player_id);
        {
            let shipwreck_id = run_shipwreck_id(&mut game, player_id);
            let world = game.app.world_mut();
            let shipwreck_entity = world
        ...
        game.tick(crate::obj::COMBAT_LOCK_TICKS as u32 + 1);
        let (burrow_entity, burrow_id, food_item_id, drink_item_id, hatchet_item_id) = {
            let world = game.app.world_mut();
            let burrow = {
                let mut burrows = world.query_filtered::<
                    (Entity, &Id, &PlayerId, &Template, &Position),
                ...
                burrows
                    .iter(world)
                    .find(|(_, _, owner, template, pos)| {
                        owner.0 == player_id && template.0 == "Burrow" && **pos == burrow_pos
                    })
                    .map(|(entity, id, ..)| (entity, id.0))
            ...
            let hero_entity = world
                .resource::<EntityObjMap>()
                .get_entity(hero_id)
                .expect("headless hero entity for Burrow supply transfer");
            let item_templates = world.resource::<Templates>().item_templates.clone();
            let food_item_id = world.resource_mut::<Ids>().new_item_id();
            let drink_item_id = world.resource_mut::<Ids>().new_item_id();
            let hatchet_item_id = world.resource_mut::<Ids>().new_item_id();
            let mut hero_inventory = world
                .get_mut::<Inventory>(hero_entity)
                .expect("headless hero inventory for Burrow supply transfer");
            hero_inventory.new(
                food_item_id,
            ...
            *world
                .get_mut::<Position>(hero_entity)
                .expect("headless hero position for Burrow supply transfer") = burrow_pos;
            (
                burrow.0,
                burrow.1,
                food_item_id,
        ...
        for item_id in [food_item_id, drink_item_id, hatchet_item_id] {
            game.inject(PlayerEvent::ItemTransfer {
                player_id,
                item_id,
                source_id: hero_id,
        ...
        game.tick(3);
        assert_eq!(
            crate::game::burrow_supply_type_count(
                game.world()
                    .get::<Inventory>(burrow_entity)
                    .expect("stocked Burrow inventory")
            ),
            BURROW_SUPPLY_GOAL
        );
        assert!(
            game.world()
                .resource::<Objectives>()
                .get(&player_id)
                .expect("objectives after Burrow supply transfer")
                .stock_burrow,
            "the transfer itself should complete the Burrow supplies task without waiting for the periodic sampler"
        );
        {
            let hero_entity = game
                .world()
                .resource::<EntityObjMap>()
                .get_entity(hero_id)
                .expect("headless hero entity after Burrow supply transfer");
            *game
                .app
                .world_mut()
                .get_mut::<Position>(hero_entity)
                .expect("restore headless hero position") = hero_pos;
        }
        let villager_id = {
            let world = game.app.world_mut();
            let mut villagers = world.query_filtered::<
        ...
        // Exercise the revised first villager task through a real persistent
        // Log gather order, not through the later Stockade build assignment.
        let log_site = {
            let world = game.app.world_mut();
            let resources = world.resource::<Resources>();
            resources
                ...
                .find_map(|(pos, resources_on_tile)| {
                    resources_on_tile
                        .values()
                        .find(|resource| resource.res_type == crate::constants::LOG)
                        .map(|resource| (*pos, resource.name.clone()))
                })
                .expect("a forest Log site for the revised tutorial")
        };
        {
            let world = game.app.world_mut();
            world
                .resource_mut::<ResourceDiscoveries>()
                .discover(player_id, log_site.0, log_site.1);
            let hero_entity = world
                .resource::<EntityObjMap>()
                .get_entity(hero_id)
                .expect("hero for revised Logging order");
            *world
                .get_mut::<Position>(hero_entity)
                .expect("hero position for revised Logging order") = log_site.0;
            let mut objective_state = world.resource_mut::<Objectives>();
            let objectives = objective_state.entry(player_id).or_default();
            objectives.recruit_villager = true;
            objectives.prospect_forest = true;
        }
        // A Logging order issued before Follow must not skip the new control
        // lesson, even though the order itself remains valid gameplay.
        game.inject(PlayerEvent::OrderGather {
            player_id,
            source_id: villager_id,
            res_type: crate::constants::LOG.to_string(),
        });
        game.tick(3);
        assert!(
            !game
                ...
                .resource::<Objectives>()
                .get(&player_id)
                .expect("objectives after premature Logging order")
                .assign_first_villager
        );

        game.inject(PlayerEvent::OrderFollow {
            player_id,
            source_id: villager_id,
        });
        game.tick(3);
        {
            let objectives = game
                ...
                .resource::<Objectives>()
                .get(&player_id)
                .expect("objectives after Follow order");
        ...
        let villager_entity = game
            ...
            .resource::<EntityObjMap>()
            .get_entity(villager_id)
            .expect("rescued villager entity after Logging order");
        assert!(matches!(
            game.world().get::<Order>(villager_entity),
            Some(Order::Gather { res_type, .. }) if res_type == crate::constants::LOG
        ));
        assert!(
            game.world()
                .resource::<Objectives>()
                .get(&player_id)
                .expect("objectives after revised Logging order")
                .assign_first_villager
        );
        {
            let hero_entity = game
                ...
                .resource::<EntityObjMap>()
                .get_entity(hero_id)
                .expect("hero after revised Logging order");
            *game
                .app
                .world_mut()
                .get_mut::<Position>(hero_entity)
                .expect("restore hero position after revised Logging order") = hero_pos;
        }

        // This checkpoint deliberately begins from an established settlement
        // and tests Stockade construction rather than the new Day-5 deed
        // milestone, which has dedicated coverage above.
        game.app
            .world_mut()
            .resource_mut::<crate::structure::Plans>()
            .add(player_id, "Stockade".to_string(), 0, 0);
        ...
        // work counts. Pathing cadence can vary with the generated fixture and
        // villagers suspend construction overnight, so allow the next morning
        // within this otherwise bounded production smoke test.
        for _ in 0..40 {
            game.tick(30);
            if game
                .world()
        ...
        assert_eq!(initial_logs - remaining_logs, STOCKADE_LOG_COST);
        ...
        assert!(
            !objectives.build_campfire,
            "the pre-existing starter Campfire must not complete the Shelter Tent upgrade task"
        );
    ...
    fn core_gameplay_checkpoint1_smoke_reconnect_and_true_death_reset() {
        ...
        let opening_due = {
            ...
            let encounter = encounters
                .get_mut(&player_id)
                .expect("initial encounter state");
            encounter.opening_rat_ambush_armed = true;
            encounter.opening_rat_spawn_tick
        };
        run_intro_check_at_or_after(&mut game, opening_due);
        let opening_ids = game
            .world()
        ...
        assert!(!fresh_objectives.equip_sharpened_stick);
        assert!(!fresh_objectives.order_villager_to_follow);
    ...
    fn checkpoint2_short_personal_crisis_simulation() {
        ...
        // The Signs minimum is 60 seconds, but this no-wealth fixture reaches
        // 45 threat when online activity reaches its third tier at 360 seconds.
        let next_tick = game.game_tick() + (360 * TICKS_PER_SEC);
        ...
        let assault_units_before_ready = game.crisis_assault_units();
        ...
        let assault_units_after_ready = game.crisis_assault_units();
        let automatic_dusk_hordes = game.metrics().waves_survived;

        assert_eq!(final_crisis.phase, CrisisPhase::AssaultReady);
        assert_eq!(assault_units_after_ready, assault_units_before_ready);
        assert!(assault_units_after_ready.is_empty());
    ...
    fn hire_from_merchant_adds_villager() {
        ...
        );
        {
            ...
            let entity = world
                .resource::<EntityObjMap>()
                .get_entity(target_id)
                .expect("hired villager entity");
            let inventory = world
                .get::<Inventory>(entity)
                .expect("hired villager inventory");
            assert_equipped_tattered_clothing(inventory);
        }
    ...
    fn upgrade_sanctuary_raises_level_and_radius() {
        use crate::game::{sanctuary_radius, Monolith, SANCTUARY_UPGRADE_COST};
        use crate::ids::Ids;
        use crate::templates::Templates;
        ...
        assert!(
            sanctuary_radius(level1) > sanctuary_radius(level0),
    ...
    fn valid_preparation_test_launches(
        ...
        let mut treatment = PreparationPairLaunch {
        ...
        treatment.common_fingerprint.crisis_pressure =
            expected_preparation_treatment_pressure(&control, &treatment);
        (control, treatment)
    }
    ...
    fn production_bandage_use_heals_consumes_once_and_records_active_assault_usage() {
        ...
        game.tick((BANDAGE_USE_TICKS + 4) as u32);
    ...
    fn production_bandage_use_at_full_health_is_a_non_consuming_noop() {
        ...
        game.tick((BANDAGE_USE_TICKS + 3) as u32);
    ...
    #[test]
    fn production_bandage_use_is_interrupted_by_combat_without_healing_or_consuming() {
        let mut game = HeadlessGame::new(100);
        let player_id = game.spawn_hero("Warrior", "InterruptedBandageUse");
        let (hero_id, bandage_id, wounded_hp) = {
            let world = game.app.world_mut();
            let item_templates = world.resource::<Templates>().item_templates.clone();
            let bandage_id = world.resource_mut::<Ids>().new_item_id();
            let mut heroes = world
            ...
            let (_, mut stats, mut inventory) = heroes
                .iter_mut(world)
                .find(|(owner, ..)| owner.0 == player_id)
                .expect("hero for interrupted bandage fixture");
            stats.hp = stats.base_hp - 20;
            let wounded_hp = stats.hp;
            let hero_id = inventory.owner;
            inventory.new(bandage_id, "Crude Bandage".to_string(), 1, &item_templates);
            (hero_id, bandage_id, wounded_hp)
        };
        ...
        assert_eq!(
            game.observe().hero.expect("bandaging hero").state,
            State::Healing
        );
        let hero_entity = game
            .app
...

        {
            ...
            world
                .get_mut::<LastCombatTick>(hero_entity)
                .expect("hero combat tick")
                .0 = current_tick;
        }
        game.tick(2);

        assert_eq!(
            game.observe().hero.expect("interrupted hero").state,
            State::None
        );
        assert!(game
            .app
        ...
        game.tick((BANDAGE_USE_TICKS + 4) as u32);
        let after = game.observe();
        assert_eq!(after.hero.expect("hero after interruption").hp, wounded_hp);
        assert_eq!(
            after
                .inventory
                .iter()
                .find(|item| item.id == bandage_id)
                .map(|item| item.quantity),
            Some(1)
        );
    }
    ...
    #[test]
    fn production_bandage_use_blocks_other_consumables_until_completion() {
        let mut game = HeadlessGame::new(100);
        let player_id = game.spawn_hero("Warrior", "ExclusiveBandageUse");
        let (hero_id, potion_id) = add_test_health_potion(&mut game, player_id, 1);
        let (bandage_id, wounded_hp) = {
            let world = game.app.world_mut();
            let item_templates = world.resource::<Templates>().item_templates.clone();
            let bandage_id = world.resource_mut::<Ids>().new_item_id();
            let hero_entity = world
                ...
                .expect("hero entity");
            let mut hero = world
                .query::<(&mut Stats, &mut Inventory)>()
                .get_mut(world, hero_entity)
                .expect("hero healing fixture");
            hero.0.hp = hero.0.base_hp - 60;
            let wounded_hp = hero.0.hp;
            hero.1
                .new(bandage_id, "Crude Bandage".to_string(), 1, &item_templates);
            (bandage_id, wounded_hp)
        };
        ...
        let after = game.observe();
        assert_eq!(
            after.hero.expect("hero after bandaging").hp,
            wounded_hp + 10
        );
        assert_eq!(
            after
                ...
                .map(|item| item.quantity),
            Some(1),
            "the queued potion must not steal the bandage's Healing state"
        );
        assert!(after
            .inventory
            .iter()
            .all(|item| item.id != bandage_id || item.quantity == 0));
    }
