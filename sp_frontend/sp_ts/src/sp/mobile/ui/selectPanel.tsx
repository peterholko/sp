...
import { TILE, OBJ, LARGE_SCREEN_WIDTH, DEAD, CORPSE } from "../../core/config";
...
export default class SelectPanel extends React.Component<SelectPanelProps, any> {
  ...
  handleDamage(eventData) {
    for(var i = 0; i < this.props.objIdsOnTile.length; i++) {
      if(eventData.state == DEAD && eventData.target_id == this.props.objIdsOnTile[i]) {
  ...
  render() {
    ...
    for (var i = this.state.startIndex; i < maxIndex; i++) {
      ...
      if (objId in Global.objectStates) {
        ...
        var isDead = Global.objectStates[objId].state == DEAD && Global.objectStates[objId].class != CORPSE;
